...
Run explicitly after seed_content. This command assigns the reference photos
and publishes the house reviews transcribed from the supplied design.
"""
...
HOUSE_REVIEWS = {
    "Первый домик": [
        ...
        (62, "Михаил Б.", "Были в июне 2025 года. Завораживающий вид, классное расположение, уют, комфорт\nв сочетании с радушной встречей оставили обалденные впечатления! В домиках чисто, белье новое, есть вся утварь для приготовления."),
    ],
    "Второй домик": [
    ...
    "Третий домик": [
        (77, "Марина Т.", "Чудесное уникальное место! Большая территория без заборов, сквозь панорамные окна видна красивая природа. Место, где мы с семьей полностью расслабились за три дня. Есть ферма, где можно погладить козу, угостить яблоками теленка."),
    ...
    "Четвёртый домик": [
        (85, "Николай Л.", "Все отлично, домики уютные и чистые, имеется все необходимое, кровать удобная, что очень важно! Чувствуется что хозяева хотели создать уют как в домиках, так и в зоне отдыха на улице. Природа шикарная, тишина и чистый воздух!"),
        (86, "Николай Л.", "Отличное соотношение цена-качество. В домике есть все необходимое для автономного проживания и даже больше. Удобное местоположение ко всем местным локациям."),
        (87, "Екатерина П.", "Мы остались очень довольны своим пребыванием в этом доме, он безумно уютный не смотря на его габариты, чистота и порядок поддерживаются на высоком уровне. Атмосфера отличная - камин, шашлык, тишина и спокойствие."),
    ],
}
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        missing = [str(PHOTO_DIR / f"ref-{number:02d}.webp") for number in used
                   if not (PHOTO_DIR / f"ref-{number:02d}.webp").is_file()]
        ...
        with transaction.atomic():
            for number in sorted(used):
                ...
                if image is None:
                    source = PHOTO_DIR / f"ref-{number:02d}.webp"
